...
(function () {
  ...
  // viewers/caster-studio.html, …) link to grammars via ?src=../schools/<slug>/grammar.json
  // — never ?grammar_id= or ?id= (see viewers/tree-viewer.html's own comment: "the src
  // path is always ../schools/<slug>/grammar.json, the form every link on this site uses").
  ...
  // is why, on cards.html?src=../schools/bus-passengers/grammar.json, asking about "item
  // 36-38" got "tell me the name of the deck you're working with" back. Fix: read the slug
  // out of ?src= ourselves, and resolve it to the app's grammar UUID via schools/_eco_ids.json
  ...
  if (srcSlug) {
    ...
    fetch(_PFX + 'schools/_eco_ids.json').then(function (r) { return r.json(); }).then(function (j) {
