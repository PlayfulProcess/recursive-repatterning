 ...
 * needed, since the source decks live right here in `schools/<slug>/grammar.json`. Mirrors the
...
(function (global) {
  ...
  function loadDeckGrammar(slug) {
    if (!deckGrammarCache[slug]) {
      deckGrammarCache[slug] = fetch('../schools/' + encodeURIComponent(slug) + '/grammar.json')
