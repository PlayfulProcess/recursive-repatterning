...
window.expandCourseEmbeds = async function(root){
  if(!root) return;
                  ...
                  if(peopleEls.length || root.querySelector('[data-embed="decks"]')){
                    PG=await get(T+'/people-of-schools/grammar.json');
                  ...
                  if(numEls.length){
                    ...
                    for(let n=1;n<=10;n++){ if(!nums||!nums[n])continue;
                      const cells=SUITORDER.map(cs=>{const c=(nidx[n]||{})[cs];if(!c||!c.img)return '';
                        return `<a class="c" href="../viewers/cards.html?src=../schools/${c.slug}/grammar.json&item=${encodeURIComponent(c.item)}" title="${esc(c.title)}"><img loading="lazy" src="${esc(c.img)}"><div>${esc(cs)}</div></a>`;}).join('');
                  ...
                  if(cardEls.length){
                    const syn=await get(RES+'/synthesis/trumps.json'); const col=await get(T+'/_collection.json'); const ecoIds=await get(T+'/_eco_ids.json');
                    for(const el of cardEls){
                      ...
                      if(col){ const cells=[];
                        for(const g of col.grammars.filter(x=>!x.is_meta)){ const dg=await get(T+'/'+g.slug+'/grammar.json'); if(!dg)continue;
                          ...
                          const url=`../viewers/cards.html?src=../schools/${g.slug}/grammar.json&item=${encodeURIComponent(it?it.id:'')}`;
                          if(img) cells.push({y:g.year||9999,slug:g.slug,h:`<a class="c" href="${url}" title="Open the ${esc(dname)} card"><img loading="lazy" src="${esc(img)}"><div>${esc(dname.slice(0,14))}</div></a>`});
                        }
                        cells.sort((a,b)=>a.y-b.y); html+=cells.map(c=>c.h).join('');
                        const withId=cells.find(c=>ecoIds&&ecoIds.ids&&ecoIds.ids[c.slug]);
                        if(withId) readingId=ecoIds.ids[withId.slug];
                      }
                      html+='</div>';
                      const lbl=tk.replace(/-/g,' ').replace(/\b\w/g,c=>c.toUpperCase());
                      const readingUrl=readingId?`https://flow.recursive.eco/g/${readingId}?view=reading`:'https://flow.recursive.eco';
                      html+=`<p class="card-links screen-only"><a href="../viewers/prototypes/lenses.html?card=${encodeURIComponent(tk)}">See ${esc(lbl)} across every deck in Lenses →</a> · <a href="${readingUrl}" target="_blank" rel="noopener">Get a reading at recursive.eco →</a></p>`;
                      el.innerHTML=html;
                    }
                  }
                  ...
                  if(synEls.length||detEls.length||sdetEls.length){
                    ...
                    if(synEls.length){ let html='';
                      for(const tk of ORDER){ if(!syn||!syn[tk])continue;
                        const entries=(idx[tk]||[]).filter(c=>c.img).sort((a,b)=>a.year-b.year);
                        const cells=entries.map(c=>`<a class="c" href="../viewers/cards.html?src=../schools/${c.slug}/grammar.json&item=${encodeURIComponent(c.id)}"><img loading="lazy" src="${esc(c.img)}"><div>${esc(c.name.slice(0,14))}</div></a>`).join('');
                        const withId=entries.find(c=>ecoIds&&ecoIds.ids&&ecoIds.ids[c.slug]);
                        const readingUrl=withId?`https://flow.recursive.eco/g/${ecoIds.ids[withId.slug]}?view=reading`:'https://flow.recursive.eco';
                        html+=`<div class="card-chapter"><h3>${NUM[tk]} — ${esc(lbl(tk))}</h3><div class="synthbox"><p>${mdl(syn[tk])}</p></div><div class="strip">${cells}</div><p class="card-links screen-only"><a href="../viewers/prototypes/lenses.html?card=${tk}">See ${esc(lbl(tk))} across every deck in Lenses →</a> · <a href="${readingUrl}" target="_blank" rel="noopener">Get a reading at recursive.eco →</a></p></div>`;
                      }
                    ...
                    if(detEls.length){ const ANCH=['cary-yale-visconti-tarot','visconti-sforza-tarot','charles-vi-tarot','este-tarot']; let html='';
                      for(const tk of ORDER){ if(!syn||!syn[tk])continue;
                        ...
                        for(const slug of [early,'tarot-de-marseille-conver','golden-dawn-book-t-tarot']){ if(!slug)continue; const c=entryOf(slug); const sc=sceneOf(c); if(!sc)continue;
                          ...
                          const thumb=c.img?`<a class="cardthumb" href="../viewers/cards.html?src=../schools/${slug}/grammar.json&item=${encodeURIComponent(c.id)}"><img loading="lazy" src="${esc(c.img)}"></a>`:'';
                  ...
                  if(plateEls.length){
                    ...
                    async function gallery(list){ let h='';
                      for(const [slug,spec,cap] of list){ const dg=await get(T+'/'+slug+'/grammar.json'); if(!dg)continue;
                        ...
                        const href=`../viewers/cards.html?src=${encodeURIComponent('../schools/'+slug+'/grammar.json')}&item=${encodeURIComponent(it.id)}`;
