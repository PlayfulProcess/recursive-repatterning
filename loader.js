/* The Recursive Tarot — tiny static loader.
   ...
   (?path=schools/<slug>&ref=<branch>). No dependencies. */
(function (g) {
  'use strict';
  ...
  function grammarUrl() {
    var file = qs('file');                 // e.g. schools/visconti-sforza-schools/grammar.json
    if (file) return rel(file);
    var path = qs('path'), ref = qs('ref'); // e.g. path=schools/<slug> ref=my-branch
    if (path) {
  ...
  // The viewer pages live at the repo root, alongside the schools/ data folder,
  ...
  function collectionUrl() {
    var ref = qs('ref');
    if (ref) return 'https://raw.githubusercontent.com/' + OWNER + '/' + REPO + '/' + ref + '/schools/_collection.json';
    return rel('schools/_collection.json');
  }
  ...
  function deckHref(slug) {
    var ref = qs('ref');
    return 'deck.html?file=schools/' + slug + '/grammar.json' + (ref ? '&ref=' + encodeURIComponent(ref) : '');
  }
